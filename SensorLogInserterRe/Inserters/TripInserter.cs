...
namespace SensorLogInserterRe.Inserters
{
    class TripInserter
    {
        ...
        public static void InsertTrip(InsertDatum datum, InsertConfig config)
        {
            ...
            {
                tripsRawTable = TripsRawLpfExDao.Get(datum);
            }
            else
            {
        ...
        private static void InsertOutwardTrip(DataTable tripsRawTable, DataTable tripsTable, InsertDatum datum, int i, InsertConfig config)
        {
            ...
            while (j < tripsRawTable.Rows.Count && tripChangeFlag == false)
            {
                ...
                if (IsYnu(tripsRawTable.Rows[j].Field<double>(TripsRawDao.ColumnEndLatitude), tripsRawTable.Rows[j].Field<double>(TripsRawDao.ColumnEndLongitude)))
                {
                    ...
                    {
                        row.SetField(TripsDao.ColumnTripId, TripsLpfExDao.GetMaxTripId() + 1);
                    }
                    else {
                    ...
                    else if (config.Correction == InsertConfig.GpsCorrection.LpfEx && !TripsLpfExDao.IsExsistsTrip(row))
                    {
                        tripsTable.Rows.Add(row);
                    }
                    else
